...
# =========================
def classify_job_category(job_title: str, job_description: str = "") -> str:
    """
    ...
    categories = {
        "Technology & IT": [
            "developer", "engineer", "programmer", "data", "tech", "it", "software", "system",
            "web", "mobile", "database", "devops", "security", "network", "cloud"
        ],
        "Sales & Marketing": [
            "sales", "marketing", "account", "business development", "bd", "commercial", 
            "promotion", "brand", "digital marketing", "seo", "social media"
        ],
        "Customer Service": [
            "customer", "service", "support", "client", "help", "reception", "guest",
            "concierge", "relations", "care"
        ],
        "Operations & Management": [
            "manager", "director", "operations", "supervisor", "coordinator", "lead",
            "chief", "head", "executive", "admin", "administration"
        ],
        "Human Resources": [
            "hr", "human resources", "recruitment", "talent", "people", "employee",
            "payroll", "benefits", "training"
        ],
        "Finance & Accounting": [
            "finance", "accounting", "accountant", "financial", "budget", "audit",
            "controller", "analyst", "treasury"
        ],
        "Hospitality & Tourism": [
            "hotel", "resort", "hospitality", "tourism", "travel", "guest", "front desk",
            "housekeeping", "restaurant", "food", "beverage", "entertainment", "animation"
        ],
        "Creative & Design": [
            "design", "creative", "graphic", "ui", "ux", "artist", "content", "writer",
            "photographer", "video", "multimedia"
        ],
        "Legal & Compliance": [
            "legal", "lawyer", "compliance", "regulatory", "risk", "audit", "contract"
        ],
        "Healthcare & Safety": [
            "health", "safety", "medical", "nurse", "doctor", "wellness", "security"
        ]
    }
    
    # Check title and description against categories
...
def clean_numeric_value(value, data_type="int"):
    """
    Clean and convert numeric values from GPT response
    """
    ...
    try:
        if data_type == "int":
            # For integers, take the first number found
            numbers = re.findall(r'\d+', str_value)
            if numbers:
                return int(numbers[0])
            return None
        elif data_type == "float":
            # For floats, try to convert directly
...
def add_provenance(df: pd.DataFrame, role_name: str, jd_text: str) -> pd.DataFrame:
    ...
    # Add job category
    job_category = classify_job_category(role_name, jd_text)
    ...
    if "AI Impact Score (0–100)" in df.columns:
        df["AI Impact Score (0–100)"] = df["AI Impact Score (0–100)"].apply(
            lambda x: clean_numeric_value(x, "int")
        )
    
    if "Task Transformation %" in df.columns:
        df["Task Transformation %"] = df["Task Transformation %"].apply(
            lambda x: clean_numeric_value(x, "float")
        )
...
def ensure_sql_schema(engine: sa.Engine):
    with engine.begin() as conn:
        ...
        try:
            conn.execute(text("ALTER TABLE all_jobs ADD COLUMN job_category TEXT;"))
        except Exception:
            pass  # Column already exists
        
        # Change ai_impact_score to INTEGER if it's not already
        try:
            conn.execute(text("ALTER TABLE all_jobs ALTER COLUMN ai_impact_score TYPE INTEGER USING ai_impact_score::integer;"))
        except Exception:
            pass  # Already correct type or conversion failed
        
        # Change task_transformation to REAL (float) if it's not already
        try:
            conn.execute(text("ALTER TABLE all_jobs ALTER COLUMN task_transformation TYPE REAL USING task_transformation::real;"))
        except Exception:
            pass  # Already correct type or conversion failed
...
def upsert_all_jobs_sql(engine: sa.Engine, df: pd.DataFrame):
    ...
    # Debug print
    if rows:
        print("DEBUG first row ai_impact_score:", rows[0].get("ai_impact_score"))
        print("DEBUG first row task_transformation:", rows[0].get("task_transformation"))

    with engine.begin() as conn:
        stmt = text("""
            INSERT INTO all_jobs
            (job_title, job_category, task, time_allocation, ai_impact_score, impact_explanation,
             task_transformation, tooling_nature, run_id, jd_hash, task_norm)
            VALUES (:job_title, :job_category, :task, :time_allocation, :ai_impact_score, :impact_explanation,
                    :task_transformation, :tooling_nature, :run_id, :jd_hash, :task_norm)
...
AI Impact Score – how strongly Gen-AI could affect the task (0 = no impact, 100 = fully automatable/augmented). IMPORTANT: Return only the numeric integer value (e.g., 75, not "75%").
Impact Explanation – 2–3 sentences justifying the chosen score. 
Task Transformation % – proportion of the task likely to change for the employee (e.g., 70% up-skilling vs 30% pure automation). IMPORTANT: Return only the numeric decimal value (e.g., 70.5, not "70.5%").
...
- AI Impact Score: Must be an integer between 0-100 (e.g., 85, not "85%" or "85 points")
- Task Transformation %: Must be a decimal number (e.g., 67.5, not "67.5%" or "approximately 68")
...
D. Assign the numeric scores (integers for AI Impact Score, decimals for Task Transformation %) and craft clear rationales in French. 
...
# =========================
# Data Viewer (Optional - show current data)
# =========================
st.divider()
...
try:
    with engine.connect() as conn:
        ...
        category_stats = pd.read_sql(text("""
            SELECT 
                job_category,
                COUNT(DISTINCT job_title) as job_count,
                COUNT(*) as task_count,
                AVG(ai_impact_score) as avg_ai_impact,
                AVG(task_transformation) as avg_transformation
            FROM all_jobs 
        ...
        if not category_stats.empty:
            ...
            with col1:
                st.dataframe(category_stats.round(1), use_container_width=True)
            
            with col2:
                # Show recent additions
                recent_jobs = pd.read_sql(text("""
                    SELECT DISTINCT job_title, job_category, run_id 
                    FROM all_jobs 
                    ORDER BY id DESC 
                    LIMIT 10
                """), conn)
                st.markdown("**Recent Jobs Added:**")
                st.dataframe(recent_jobs, use_container_width=True)
        else:
            st.info("No data available yet. Generate some reports to see statistics!")

except Exception as e:
    st.warning(f"Could not load data overview: {e}")
...
with col1:
    if st.button("📊 Export All Data to Excel"):
        try:
            with engine.connect() as conn:
                all_data = pd.read_sql(text("SELECT * FROM all_jobs ORDER BY id DESC"), conn)
                synthesis_data = pd.read_sql(text("SELECT * FROM synthesis ORDER BY id DESC"), conn)
            
            if not all_data.empty:
                # Create Excel export
                export_buffer = io.BytesIO()
                with pd.ExcelWriter(export_buffer, engine='openpyxl') as writer:
                    all_data.to_excel(writer, sheet_name='All Jobs', index=False)
                    synthesis_data.to_excel(writer, sheet_name='Synthesis', index=False)
                
                export_buffer.seek(0)
                st.download_button(
                    label="📥 Download Excel Export",
                    data=export_buffer,
                    file_name=f"genai_job_analysis_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
            else:
...
with col2:
    if st.button("🗑️ Clear All Data"):
        if st.checkbox("I confirm I want to delete all data"):
            try:
                with engine.begin() as conn:
                    conn.execute(text("DELETE FROM all_jobs"))
...
st.markdown(
    ...
    <div style='text-align: center; color: #666; font-size: 0.9em;'>
        💼 GenAI Job Impact Analyst | Built with Streamlit & PostgreSQL on Render<br>
        Analyzing the future of work with AI at Club Med
    </div>
    """, 
    unsafe_allow_html=True
)
